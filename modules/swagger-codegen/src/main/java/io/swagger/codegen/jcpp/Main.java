...
public class Main {
    ...
    public void run(String[] args) throws Exception {
        ...
        OptionSpec<?> noLineMarks = parser.acceptsAll(Arrays.asList("P"),
                "Disables linemarks in the output.");
        OptionSpec<?> skipNewLines = parser.acceptsAll(Arrays.asList("S"),
                "Skip new lines of preprocessor directives.");
        ...
        if (!options.has(noLineMarks))
            pp.addFeature(Feature.LINEMARKERS);

        if (options.has(skipNewLines))
            pp.addFeature(Feature.SKIPNEWLINES);
        ...
        try {
            for (;;) {
                ...
                if (tok.getType() == Token.EOF)
                    break;
                if (tok.getType() != Token.P_NL || !pp.getFeature(Feature.SKIPNEWLINES))
